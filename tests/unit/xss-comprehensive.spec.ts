...
describe("Comprehensive XSS Protection", () => {
    describe("Script Injection Attacks", () => {
        ...
        it("should detect script with attributes", () => {
            expect(containsXssAttempt('<script src="evil.js"></script>')).toBe(
                true,
            );
            expect(
                containsXssAttempt(
    ...
    describe("Protocol-Based Attacks", () => {
        ...
        it("should detect data: URIs with HTML", () => {
            expect(
                containsXssAttempt("data:text/html,<script>alert(1)</script>"),
            ).toBe(true);
            expect(
                containsXssAttempt(
    ...
    describe("Dangerous HTML Tags", () => {
        it("should detect iframe tags", () => {
            expect(containsXssAttempt('<iframe src="evil.com"></iframe>')).toBe(
                true,
            );
            expect(containsXssAttempt('<IFRAME src="evil.com"></IFRAME>')).toBe(
                true,
        ...
        it("should detect object tags", () => {
            expect(
                containsXssAttempt('<object data="evil.swf"></object>'),
            ).toBe(true);
            expect(
                containsXssAttempt('<OBJECT data="evil.swf"></OBJECT>'),
            ).toBe(true);
        ...
        it("should detect applet tags", () => {
            expect(
                containsXssAttempt('<applet code="Evil.class"></applet>'),
            ).toBe(true);
        });
        ...
        it("should detect link tags", () => {
            expect(
                containsXssAttempt('<link rel="stylesheet" href="evil.css">'),
            ).toBe(true);
        });
        ...
        it("should detect SVG tags", () => {
            ...
            expect(
                containsXssAttempt("<svg><script>alert(1)</script></svg>"),
            ).toBe(true);
        });

        it("should detect MathML tags", () => {
            expect(containsXssAttempt("<math><mtext>test</mtext></math>")).toBe(
                true,
            );
        });
    });
    ...
    describe("HTML Entity Encoding", () => {
        it("should properly escape HTML special characters", () => {
            ...
            expect(sanitizeHtml("Test & <danger>")).toBe(
                "Test &amp; &lt;danger&gt;",
            );

            // Quotes are preserved by textContent (safe for display)
            expect(sanitizeHtml("\"quotes\" and 'apostrophes'")).toContain(
                '"quotes"',
            );
        });
    ...
    describe("Context-Specific Protection", () => {
        ...
        it("should handle error messages safely", () => {
            const testCases = [
                {
                    msg: "Error: <script>alert(1)</script>",
                    expect: "&lt;script",
                },
                {
                    msg: 'Failed to load: <img onerror="alert(1)" src=x>',
                    expect: "&lt;img",
                },
                {
                    msg: "Invalid input: javascript:alert(1)",
                    expect: "javascript:",
                },
            ];
