...
const distFile = path.join(
    process.cwd(),
    "packages/api/dist/functions/validation-ossf.js",
);
let handler;
...
describe("validation-ossf function", () => {
    beforeAll(() => {
        if (!fs.existsSync(distFile))
            throw new Error("validation-ossf dist missing. Build first.");
    ...
    it("400 when minScore missing", async () => {
        ...
        const res = await handler(
            ctx,
            makeReq("POST", { templateUrl: "owner/repo" }),
        );
        expect(res.status).toBe(400);
    ...
    it("400 when minScore invalid", async () => {
        ...
        const res = await handler(
            ctx,
            makeReq("POST", { templateUrl: "owner/repo", minScore: 15 }),
        );
        expect(res.status).toBe(400);
    });
});
