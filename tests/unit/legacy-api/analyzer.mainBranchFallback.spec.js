...
const analyzerSource = fs.readFileSync(
    path.join(process.cwd(), "packages/app/js/analyzer.js"),
    "utf8",
);
...
function loadClasses() {
    ...
    global.TemplateAnalyzerDocs.prototype.getConfig = async () => ({
        requiredFiles: [],
    });
    ...
    TemplateAnalyzer =
        global.TemplateAnalyzer || global.window.TemplateAnalyzer;
...
describe("Analyzer main branch fallback", () => {
    ...
    it("falls back when main missing", async () => {
        ...
        global.fetch = vi.fn(async (url) => {
            ...
            calls.push(url);
            if (/dod-config\.json/.test(url))
                return { ok: true, json: async () => ({ requiredFiles: [] }) };
            if (/partner-config\.json/.test(url))
                return { ok: true, json: async () => ({ requiredFiles: [] }) };
            if (/custom-config\.json/.test(url))
                return { ok: true, json: async () => ({ requiredFiles: [] }) };
        ...
        if (analyzer && analyzer.ruleSetConfigs) {
            ...
            cfg.requiredFiles = Array.isArray(cfg.requiredFiles)
                ? cfg.requiredFiles
                : [];
            cfg.requiredFolders = Array.isArray(cfg.requiredFolders)
                ? cfg.requiredFolders
                : [];
            cfg.requiredWorkflowFiles = Array.isArray(cfg.requiredWorkflowFiles)
                ? cfg.requiredWorkflowFiles
                : [];
        }
        // inject minimal DOM support for analyzer UI operations
        const result = await analyzer.analyzeTemplate(
            "https://github.com/SomeOrg/sample",
            "dod",
        );
