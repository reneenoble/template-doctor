...
describe("Input Validation UX - Consistent Behavior", () => {
    ...
    describe("XSS Detection", () => {
        it("should detect script tags", () => {
            expect(containsXssAttempt('<script>alert("XSS")</script>')).toBe(
                true,
            );
            expect(containsXssAttempt('<ScRiPt>alert("XSS")</ScRiPt>')).toBe(
                true,
        ...
        it("should not flag safe input", () => {
            expect(containsXssAttempt("https://github.com/owner/repo")).toBe(
                false,
            );
    ...
    describe("URL Validation", () => {
        it("should accept valid GitHub URLs", () => {
            expect(sanitizeGitHubUrl("https://github.com/owner/repo")).toBe(
                "https://github.com/owner/repo",
            );
            expect(sanitizeGitHubUrl("https://github.com/owner/repo.git")).toBe(
                "https://github.com/owner/repo",
            );
            expect(sanitizeGitHubUrl("owner/repo")).toBe(
                "https://github.com/owner/repo",
            );
        });
        ...
        it("should reject XSS attempts in URLs", () => {
            ...
            expect(
                sanitizeGitHubUrl('owner/repo" onclick="alert(1)'),
            ).toBeNull();
        });
    ...
    describe("Error Message Consistency", () => {
        ...
        it("should show format guidance for single words", () => {
            const singleWords = ["hello", "randomword", "test123"];
    ...
    describe("Red Border Behavior", () => {
        ...
        it("should determine need for red border on invalid URLs", () => {
            ...
            const invalidUrls = lines.filter(
                (line) => !sanitizeGitHubUrl(line.trim()),
            );
