...
describe('Analyzer category grouping', () => {
  it('groups issues and compliant items into categories', async () => {
    ...
    const result = await runAnalyzer('https://github.com/test/repo', mockFiles, {
      ruleSet: 'dod',
    });
    ...
    for (const category of expectedCategories) {
      ...
      expect(result.compliance.categories[category]).toHaveProperty('enabled');
      expect(result.compliance.categories[category]).toHaveProperty('issues');
      expect(result.compliance.categories[category]).toHaveProperty('compliant');
      expect(result.compliance.categories[category]).toHaveProperty('percentage');
      expect(Array.isArray(result.compliance.categories[category].issues)).toBe(true);
      expect(Array.isArray(result.compliance.categories[category].compliant)).toBe(true);
      expect(typeof result.compliance.categories[category].percentage).toBe('number');
    }
  });
  ...
  it('correctly maps issue categories to standard category keys', async () => {
    ...
    const missingFileIssues = repoMgmt.issues.filter(
      (i) =>
        i.id && (i.id.includes('missing') || i.id.includes('required') || i.id.includes('file')),
    );
  ...
  it('calculates correct percentage for each category', async () => {
    ...
    for (const categoryKey of Object.keys(result.compliance.categories)) {
      ...
      if (total > 0) {
        const expectedPercentage = Math.round((cat.compliant.length / total) * 100);
  ...
  it('does not include meta category in tiles', async () => {
    ...
    const metaItem = result.compliance.compliant.find((i) => i.category === 'meta');
...
resource storage 'Microsoft.Storage/storageAccounts@2021-06-01' = {
  properties: {
    ...
    const totalSecurityItems = securityCat.issues.length + securityCat.compliant.length;
  ...
  it('groups deployment-related items into deployment category', async () => {
    ...
    const totalDeploymentItems = deploymentCat.issues.length + deploymentCat.compliant.length;
    expect(totalDeploymentItems).toBeGreaterThan(0);

    // Should include bicep or azure.yaml items
    const allDeploymentItems = [...deploymentCat.issues, ...deploymentCat.compliant];
    const deploymentRelated = allDeploymentItems.some(
      (i) =>
        i.id && (i.id.includes('bicep') || i.id.includes('azure') || i.id.includes('workflow')),
    );
