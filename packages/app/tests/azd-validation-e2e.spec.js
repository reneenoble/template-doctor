...
test.describe('Artifact-Based Validation Results', () => {
  ...
  test('should display warning status with artifact data', async ({ page }) => {
    ...
    // Verify AZD times
    const azdUp = await page.locator('.validation-step').filter({ hasText: 'AZD Up' }).textContent();
    expect(azdUp).toContain('45.2s');

    const azdDown = await page.locator('.validation-step').filter({ hasText: 'AZD Down' }).textContent();
    expect(azdDown).toContain('30.1s');

    // Verify security warnings
    const security = await page.locator('.validation-step').filter({ hasText: 'Security' }).textContent();
...
test.describe('GraphQL Issue Creation', () => {
  ...
  test('falls back to URL form if GraphQL unavailable', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      if (window.GitHubClient) {
        try { delete window.GitHubClient; } catch { window.GitHubClient = undefined; }
      }
    });

    await page.route('**/api/v4/validation-template', (route) => {
      route.fulfill({ json: { runId: 'run-fallback', githubRunId: 1, githubRunUrl: 'https://github.com/test/repo/actions/runs/1' } });
    });
    await page.route('**/api/v4/validation-status*', (route) => {
      route.fulfill({ json: { status: 'completed', conclusion: 'failure', html_url: 'https://github.com/test/repo/actions/runs/1', azdValidation: { azdUpSuccess: false, azdUpTime: null, azdDownSuccess: false, azdDownTime: null, psRuleErrors: 0, psRuleWarnings: 0, securityStatus: 'errors', overallStatus: 'failure', resultFileContent: 'Failure' } } });
    });
    await page.evaluate(() => {
      document.dispatchEvent(new CustomEvent('template-card-validate', { detail: { template: { repoUrl: 'https://github.com/test/repo' } } }));
    });
    ...
    await page.addInitScript(() => {
      ...
      window.open = function(u, t) { window.__openedUrls.push(u); return typeof orig === 'function' ? orig.call(window, u, t) : null; };
    });
    await page.click('button:has-text("Create GitHub Issue")');
  ...
  let inputPayload = null;
    await page.route('**/graphql', (route) => {
      ...
      route.fulfill({ json: { data: { createIssue: { issue: { id: 'id1', number: 77, url: 'https://github.com/test/repo/issues/77', title: 't' } } } } });
    });
    await page.route('**/api/v4/validation-template', (route) => route.fulfill({ json: { runId: 'run-copilot', githubRunId: 2, githubRunUrl: 'https://github.com/test/repo/actions/runs/2' } }));
    await page.route('**/api/v4/validation-status*', (route) => route.fulfill({ json: { status: 'completed', conclusion: 'failure', html_url: 'https://github.com/test/repo/actions/runs/2', azdValidation: { azdUpSuccess: false, azdUpTime: null, azdDownSuccess: false, azdDownTime: null, psRuleErrors: 1, psRuleWarnings: 0, securityStatus: 'errors', overallStatus: 'failure', resultFileContent: '(x) Failed: X' } } }));
    await page.evaluate(() => { document.dispatchEvent(new CustomEvent('template-card-validate', { detail: { template: { repoUrl: 'https://github.com/test/repo' } } })); });
    await page.waitForSelector('.validation-failure');
    await page.click('button:has-text("Create GitHub Issue")');
    await page.waitForTimeout(500);
  ...
  test('elapsed time spinner replaced by clock after completion', async ({ page }) => {
    await page.route('**/api/v4/validation-template', r => r.fulfill({ json: { runId: 'run-time', githubRunId: 3, githubRunUrl: 'https://github.com/test/repo/actions/runs/3' } }));
    let poll = 0;
    await page.route('**/api/v4/validation-status*', r => {
      poll++;
      if (poll < 2) return r.fulfill({ json: { status: 'in_progress', conclusion: null, html_url: 'https://github.com/test/repo/actions/runs/3' } });
      return r.fulfill({ json: { status: 'completed', conclusion: 'success', html_url: 'https://github.com/test/repo/actions/runs/3', azdValidation: { azdUpSuccess: true, azdUpTime: '5s', azdDownSuccess: true, azdDownTime: '6s', psRuleErrors: 0, psRuleWarnings: 0, securityStatus: 'pass', overallStatus: 'success', resultFileContent: '' } } });
    });
    await page.evaluate(() => { document.dispatchEvent(new CustomEvent('template-card-validate', { detail: { template: { repoUrl: 'https://github.com/test/repo' } } })); });
  ...
  test('elapsed time formatting for short (<60s) and long (>60s) durations', async ({ page }) => {
    ...
    await page.route('**/api/v4/validation-template', r => r.fulfill({ json: { runId: 'run-durations', githubRunId: 4, githubRunUrl: 'https://github.com/test/repo/actions/runs/4' } }));
    let poll = 0;
    await page.route('**/api/v4/validation-status*', r => {
      poll++;
      if (poll < 3) return r.fulfill({ json: { status: 'in_progress', conclusion: null, html_url: 'https://github.com/test/repo/actions/runs/4' } });
      return r.fulfill({ json: { status: 'completed', conclusion: 'success', html_url: 'https://github.com/test/repo/actions/runs/4', azdValidation: { azdUpSuccess: true, azdUpTime: '10s', azdDownSuccess: true, azdDownTime: '12s', psRuleErrors: 0, psRuleWarnings: 0, securityStatus: 'pass', overallStatus: 'success', resultFileContent: '' } } });
    });
    await page.evaluate(() => { document.dispatchEvent(new CustomEvent('template-card-validate', { detail: { template: { repoUrl: 'https://github.com/test/repo' } } })); });
    // Advance 30s
