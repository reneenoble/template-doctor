...
class GitHubClient {
  ...
  async graphql(query, variables = {}) {
    ...
    try {
      ...
      if (!error.errors && window.Notifications) {
        window.Notifications.error(
          'Request Failed',
          'Failed to communicate with GitHub API. Please try again later.',
        );
      }
      throw error;
    }
  ...
  async listAllFiles(owner, repo, ref = 'HEAD') {
    ...
    return response.tree.filter((item) => item.type === 'blob').map((item) => item.path);
  }
  ...
  async createIssueGraphQL(owner, repo, title, body, labelNames = []) {
    ...
    try {
      ...
      if (!scopes.includes('public_repo') && !scopes.includes('repo')) {
        ...
        throw new Error(
          'Your GitHub token does not have the "public_repo" permission required to create issues',
        );
      }
      ...
      try {
        ...
        if (copilotActor) {
          ...
          if (copilotActor) {
            ...
            console.warn(
              '⚠️ Copilot agent not found in suggested actors. Issue will be created without assignment.',
            );
          }
        }
      } catch (error) {
      ...
      if (copilotActor) {
        ...
        try {
          ...
          console.log(
            `Attempting to assign issue #${issueNumber} to @copilot-agent-swe using REST API fallback...`,
          );
          ...
          if (response.ok) {
            ...
            console.warn(
              '❌ Failed to assign issue using REST API fallback:',
              await response.text(),
            );
            ...
            if (assignSuccess) {
              console.log(
                `✅ Successfully assigned issue #${issueNumber} to Copilot using GraphQL approach`,
              );
            } else {
  ...
  async assignIssueToCopilotBot(owner, repo, issueNumber) {
    try {
      ...
      if (!copilotActor) {
        console.warn(
          'Copilot bot not found in suggested actors. Make sure Copilot is enabled for this repository.',
        );
        return false;
      }
  ...
  async checkUserHasFork(owner, repo) {
    ...
    try {
      ...
      if (
        repoData.fork === true &&
        repoData.parent &&
        repoData.parent.full_name === `${owner}/${repo}`
      ) {
        return true;
      }
  ...
  async findIssuesByTitle(owner, repo, title, labelName) {
    ...
    console.log(
      `Found ${data.repository.issues.nodes.length} issues in repository, filtering for title match`,
    );
