...
async function runGithubWorkflowValidation(templateUrl, apiBase, onStatusChange) {
  ...
  console.log(`[${callId}] Full API URL:`, `${apiBaseNormalized}${apiPathPrefix}/validation-template`);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (errorData) {
          ...
          if (errorData.diagnosticInfo) {
            ...
            if (errorData.diagnosticInfo.envVars && errorData.diagnosticInfo.envVars.hasWorkflowToken !== undefined) {
              logsElem.innerHTML += `  • Token provided: ${errorData.diagnosticInfo.envVars.hasWorkflowToken ? 'Yes' : 'No'}\n`;
              
              if (errorData.diagnosticInfo.envVars.hasWorkflowToken && errorData.diagnosticInfo.envVars.tokenLength) {
      ...
      if (response.status === 400) {
        ...
        troubleshootingHints = "GitHub API communication issue. The server could not communicate with GitHub.";
      } else if (response.status === 503) {
        troubleshootingHints = "Service temporarily unavailable. Please try again later.";
      } else if (response.status === 504) {
        troubleshootingHints = "Gateway timeout. The server did not receive a timely response from GitHub.";
      } else if (response.status === 500) {
        troubleshootingHints = "Internal server error. This could be due to several issues:\n" +
    ...
    const actionUrl = data.githubRunUrl
      ? data.githubRunUrl
      : (data.githubRunId ? `https://github.com/Template-Doctor/template-doctor/actions/runs/${data.githubRunId}`
...
`;
    ...
    if (isTimeoutError) {
      ...
      errorMessage = 'The request to the server took too long to complete. The server might be overloaded or experiencing issues.';
      ...
      errorMessage = 'There was a problem communicating with GitHub. This might be due to token issues or rate limiting.';
    } else if (error.message.includes('500')) {
      errorTitle = 'Server Error';
      errorMessage = 'The server encountered an internal error. This might be due to configuration issues or temporary problems.';
    }
...
async function pollGithubWorkflowStatus(runId, templateUrl, apiBase, onStatusChange) {
  ...
  try {
    ...
    if (stored) {
      ...
      if (parsed.githubRunId) {
        ...
        actionRunUrl = parsed.githubRunUrl || `https://github.com/Template-Doctor/template-doctor/actions/runs/${knownGithubRunId}`;
      }
    }
  } catch (e) {
  ...
  const statusData = await statusResponse.json();
      ...
      if (statusData.githubRunId) {
        ...
        actionRunUrl = statusData.runUrl || `https://github.com/Template-Doctor/template-doctor/actions/runs/${knownGithubRunId}`;
        try {
          const toStore = { githubRunId: knownGithubRunId, githubRunUrl: statusData.runUrl || null };
