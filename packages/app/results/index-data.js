...
(function () {
  console.log(
    '###############[index-data] Loading template data, authentication check:',
  ...
  const requireAuth =
    typeof cfg.requireAuthForResults === 'boolean' ? cfg.requireAuthForResults : false;
  const isAuthed = !!(
    window.GitHubAuth &&
    window.GitHubAuth.isAuthenticated &&
    window.GitHubAuth.isAuthenticated()
  ...
  if (requireAuth && !isAuthed) {
    console.log(
      '[index-data] Auth required but user not authenticated – leaving templatesData populated (UI may hide it).',
    );
  ...
  console.log(
    '[index-data] Template data loaded (non-destructive). Entries:',
  ...
  try {
    if (Array.isArray(window.__TD_DYNAMIC_RESULTS) && window.__TD_DYNAMIC_RESULTS.length) {
      ...
      const toAdd = window.__TD_DYNAMIC_RESULTS.filter(
        (e) => e && !existingKeys.has(`${e.repoUrl}::${e.dashboardPath}`),
      );
      if (toAdd.length) {
        window.templatesData.push(...toAdd);
        console.log(
          `[index-data] Merged ${toAdd.length} dynamic scan entries (total now ${window.templatesData.length}).`,
        );
      }
    }
  } catch (e) {
  ...
  const disableFallback =
    typeof cfg.disableFallbackTemplates === 'boolean'
      ? cfg.disableFallbackTemplates
      : window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';

  if (
    !disableFallback &&
    (window.__TD_DYNAMIC_RESULTS == null || window.__TD_DYNAMIC_RESULTS.length === 0) &&
    window.templatesData.length === 0
  ) {
    console.warn(
      '[index-data] Dynamic meta files appear to have NOT loaded (dynamicCount=0). Using embedded fallback meta.',
    );
    ...
    try {
      if (typeof document !== 'undefined') {
        ...
        dynamicScript.onload = function () {
          console.log(
            '[index-data] Dynamically injected scan-meta-backfill.js loaded successfully',
          );
          // Try to merge again after loaded
          if (Array.isArray(window.__TD_DYNAMIC_RESULTS) && window.__TD_DYNAMIC_RESULTS.length) {
            console.log(
              '[index-data] Dynamic script loaded entries:',
            ...
            if (banner) {
              ...
              banner.textContent =
                'Dynamic load succeeded after injection! Refresh page to use dynamic data only.';
            }
          }
        };
        dynamicScript.onerror = function () {
          console.error(
            '[index-data] Failed to dynamically load scan-meta-backfill.js - fallback will remain',
          );
        };
  ...
  if (!window.__TD_INDEX_RENDER_ATTEMPTED) {
    ...
    function basicRender() {
      ...
      if (window.__TD_FALLBACK_EMBEDDED) {
        ...
        if (!banner) {
          ...
          banner.style.cssText =
            'background:#ffe08a;color:#512c00;padding:8px 12px;margin:4px 0 12px;border:1px solid #d19400;border-radius:4px;font-size:12px;font-family:monospace;';
          banner.textContent =
            'Fallback meta used (dynamic scan-meta files not loaded). Ensure scan-meta-backfill.js is present and not blocked.';
      ...
      window.templatesData.forEach((t) => {
        const p =
          t.compliance && typeof t.compliance.percentage === 'number' ? t.compliance.percentage : 0;
      ...
      console.log(
        '[index-data] Basic inline render complete (cards:',
        window.templatesData.length,
        ')',
      );
    }
