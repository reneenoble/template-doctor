...
(function () {
  const maxPerRepo =
    (window.TemplateDoctorConfig && window.TemplateDoctorConfig.maxScansPerRepo) || 8; // retention cap
  ...
  for (const entry of list) {
    ...
    if (missing.length) {
      console.warn('[meta-validator] Dropping entry missing fields', missing, entry);
      continue;
    }
    // minimal compliance shape
    if (!entry.compliance || typeof entry.compliance !== 'object') {
      console.warn('[meta-validator] Bad compliance shape', entry);
      continue;
    }
    // dedupe key
    ...
    if (!normTs) {
      console.warn('[meta-validator] Invalid timestamp', entry.timestamp);
      continue;
    }
  ...
  const byRepo = valid.reduce((m, e) => {
    (m[e.repoUrl] = m[e.repoUrl] || []).push(e);
    return m;
  }, {});
  const finalList = [];
