...
export async function runAnalyzer(
  ...
  // Check for README.md
  const readmeFile = files.find((f) => f.path && f.path.toLowerCase() === 'readme.md');
  if (readmeFile) {
    compliant.push({
      id: 'has-readme',
    ...
    if (readmeFile.content) {
      const readmeContent = Buffer.from(readmeFile.content, 'base64').toString('utf-8');
  ...
  const licenseFile = files.find(
    (f) => f.path && (f.path.toLowerCase() === 'license' || f.path.toLowerCase() === 'license.md'),
  );
  if (licenseFile) {
    compliant.push({
      id: 'has-license',
  ...
  const azureYaml = files.find(
    (f) =>
      f.path && (f.path.toLowerCase() === 'azure.yaml' || f.path.toLowerCase() === 'azure.yml'),
  );
  if (azureYaml) {
    compliant.push({
      id: 'has-azure-yaml',
    ...
    if (azureYaml.content && config.azureYamlRules?.mustDefineServices) {
      const azureYamlContent = Buffer.from(azureYaml.content, 'base64').toString('utf-8');
  ...
  const hasGithub = files.some((f) => f.path && f.path.toLowerCase().startsWith('.github/'));
  if (hasGithub) {
    compliant.push({
  ...
  const hasInfra = files.some((f) => f.path && f.path.toLowerCase().startsWith('infra/'));
  if (hasInfra) {
    const infraFiles = files.filter((f) => f.path && f.path.toLowerCase().startsWith('infra/'));
    compliant.push({
      id: 'has-infra',
      category: 'infrastructure',
    ...
    // Check Bicep files
    const bicepFiles = infraFiles.filter((f) => f.path && f.path.endsWith('.bicep'));
    if (bicepFiles.length === 0) {
      ...
      for (const file of bicepFiles) {
        if (file.content) {
          ...
          analyzeAuthenticationMethods(content, file.path, issues, compliant, config);
        }
      }
    }
  } else {
