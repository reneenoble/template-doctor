...
import { parseAzdValidationResult, downloadValidationArtifact } from '../src/services/azd-validation';
...
describe('downloadValidationArtifact (service)', () => {
  ...
  it('returns null when artifacts list fetch fails', async () => {
    ...
    const result = await downloadValidationArtifact(owner, repo, runId, token);
    expect(result).toBeNull();
  });

  it('returns null when no validation artifact found', async () => {
    global.fetch = vi.fn().mockResolvedValueOnce({ ok: true, json: async () => ({ artifacts: [{ name: 'other-artifact' }] }) });
    const result = await downloadValidationArtifact(owner, repo, runId, token);
    expect(result).toBeNull();
  });
  ...
  it('returns null when artifact download fails', async () => {
    global.fetch = vi.fn()
      // artifacts list
      .mockResolvedValueOnce({ ok: true, json: async () => ({ artifacts: [{ name: 'abc-validation-result', id: 9 }] }) })
      // artifact download
  ...
  it('extracts markdown from ZIP (md preferred)', async () => {
    const mdEntry = { entryName: 'result.md', isDirectory: false, getData: () => Buffer.from('# MD') };
    (AdmZip as any).mockImplementation(() => ({ getEntries: () => [mdEntry] }));
    global.fetch = vi.fn()
      .mockResolvedValueOnce({ ok: true, json: async () => ({ artifacts: [{ name: 'abc-validation-result', id: 9 }] }) })
      .mockResolvedValueOnce({ ok: true, arrayBuffer: async () => Buffer.from('zip') });
    const result = await downloadValidationArtifact(owner, repo, runId, token);
    expect(result).toBe('# MD');
  });

  it('falls back to .log when no .md file', async () => {
    const logEntry = { entryName: 'out.log', isDirectory: false, getData: () => Buffer.from('LOG') };
    (AdmZip as any).mockImplementation(() => ({ getEntries: () => [logEntry] }));
    global.fetch = vi.fn()
      .mockResolvedValueOnce({ ok: true, json: async () => ({ artifacts: [{ name: 'abc-validation-result', id: 9 }] }) })
      .mockResolvedValueOnce({ ok: true, arrayBuffer: async () => Buffer.from('zip') });
    const result = await downloadValidationArtifact(owner, repo, runId, token);
