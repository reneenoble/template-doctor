...
export async function requireAuth(
    ...
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
        return res.status(401).json({
            error: "Unauthorized",
            message:
                "Missing or invalid Authorization header. Expected: Bearer <github_token>",
        });
    }
...
export function requireAdmin(req: Request, res: Response, next: NextFunction) {
    ...
    if (!githubUser) {
        return res.status(401).json({
            error: "Unauthorized",
            message:
                "Authentication required. Use requireAuth middleware first.",
        });
    }

    // Get admin users from environment variable
    const adminUsers =
        process.env.ADMIN_GITHUB_USERS?.split(",").map((u) => u.trim()) || [];

    if (adminUsers.length === 0) {
        console.warn(
            "[Auth] No admin users configured. Set ADMIN_GITHUB_USERS environment variable.",
        );
        return res.status(503).json({
            error: "Service Unavailable",
            message:
                "Admin access not configured. Please contact the system administrator.",
        });
    }

    if (!adminUsers.includes(githubUser)) {
        console.warn(
            `[Auth] Unauthorized admin access attempt by: ${githubUser}`,
        );
...
export async function requireSetupAccess(
    ...
    const allowedUsers =
        process.env.SETUP_ALLOWED_USERS?.split(",").map((u) => u.trim()) || [];
