...
analyzeRouter.post('/analyze-template', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (!repoUrl) {
      return res.status(400).json({ error: 'repoUrl or repos array is required' });
    }
...
export async function analyzeSingleRepository(
  ...
  try {
    ...
    if (authenticatedUser && authenticatedUser.toLowerCase() !== owner.toLowerCase()) {
      console.log(`Attempting fork-first: checking ${authenticatedUser}/${repo}`);
      try {
        ...
        console.log(`Using fork: ${owner}/${repo} (branch: ${defaultBranch})`);
      } catch (forkErr: any) {
        console.log(`Fork not found, will try upstream: ${forkErr?.message}`);
        ...
        console.log(`Accessing upstream with ${token ? 'server' : 'user'} token`);
      ...
      console.log(`Direct access with ${userToken ? 'user' : 'server'} token`);
...
async function handleBatchAnalysis(
  ...
  console.log(`[${batchId}] Starting batch analysis of ${repos.length} repositories`);

  // Process repos sequentially to avoid overwhelming GitHub API and analyzer
  for (let i = 0; i < repos.length; i++) {
    const repoUrl = repos[i];
    console.log(`[${batchId}] Analyzing ${i + 1}/${repos.length}: ${repoUrl}`);
    ...
    try {
      ...
      console.error(`[${batchId}] Error analyzing ${repoUrl}:`, error?.message);
      results.push({
        repoUrl,
        status: 'error',
  ...
  console.log(`[${batchId}] Batch complete: ${successCount}/${repos.length} successful`);
...
function createGitHubClient(defaultToken?: string) {
  return async function gh(path: string, overrideToken?: string): Promise<any> {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        'GitHub request failed ' + res.status + ' ' + path + ' ' + text.slice(0, 200),
      );
    }
    return res.json();
  };
...
async function listAllFilesFetch(
  ...
  const apiPath = `/repos/${owner}/${repo}/contents/${encodeURIComponent(path)}`.replace(
    /%2F/g,
    '/',
  );
  const data = await gh(apiPath + (ref ? `?ref=${encodeURIComponent(ref)}` : ''), accessToken);
  ...
  for (const entry of entries) {
    if (entry.type === 'file') {
      ...
      const sub = await listAllFilesFetch(gh, owner, repo, ref, entry.path, accessToken);
