...
adminConfigRouter.get("/config", async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({
            error: "Failed to fetch configuration settings",
...
adminConfigRouter.get("/config/:key", async (req: Request, res: Response) => {
    try {
        ...
        if (value === null) {
            return res
                .status(404)
                .json({ error: "Configuration setting not found" });
        }
        ...
        res.status(500).json({
            error: "Failed to fetch configuration setting",
            message: error.message,
        });
    }
});
...
adminConfigRouter.put("/config/:key", async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({
            error: "Failed to update configuration setting",
            message: error.message,
        });
    }
});
...
adminConfigRouter.delete(
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            res.status(500).json({
                error: "Failed to delete configuration setting",
                message: error.message,
            });
        }
    },
