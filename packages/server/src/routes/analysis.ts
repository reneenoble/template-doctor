 ...
 */
router.post('/add-template-pr', async (req: Request, res: Response, next: NextFunction) => {
  const requestId = `req-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;

  try {
    const tokenHeader = req.headers['authorization'] || req.headers['Authorization'];
    const token =
      typeof tokenHeader === 'string' ? tokenHeader.replace(/Bearer\s+/i, '').trim() : undefined;
    ...
    const latestObj = createLatestJson(templateData, dataFileName, dashboardFileName);
    await octokit.repos.createOrUpdateFileContents({
      owner,
      repo,
      path: `${folderPath}/latest.json`,
      message: `Update latest.json for template: ${repoIdentifier}`,
      content: Buffer.from(JSON.stringify(latestObj, null, 2)).toString('base64'),
      branch: branchName,
      sha: latestSha,
    });
    ...
    history.push(createHistoryEntry(templateData, dataFileName, dashboardFileName));
    await octokit.repos.createOrUpdateFileContents({
      owner,
      repo,
      path: `${folderPath}/history.json`,
    ...
    const indexContent = Buffer.from(indexFile.content, 'base64').toString();
    const updatedIndex = addTemplateToIndexData(indexContent, templateData);
    await octokit.repos.createOrUpdateFileContents({
      owner,
      repo,
      path: 'packages/app/results/index-data.js',
    ...
    // Create Pull Request
    const prBody = buildPrBody(templateData, folderPath, dashboardFileName, dataFileName);
 ...
 */
router.post('/archive-collection', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const token = process.env.GH_WORKFLOW_TOKEN;
    const repoSlug =
      process.env.ARCHIVE_REPO_SLUG || 'Template-Doctor/centralized-collections-archive';
    ...
    const { collection, repoUrl, repoName, analysisId, username, timestamp, metadata } =
      req.body || {};
    ...
    // Create file
    await gh(`/repos/${repoSlug}/contents/${encodeURIComponent(archivePath)}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
function addTemplateToIndexData(content: string, templateData: any): string {
  ...
  if (arrayStart === -1) throw new Error('Could not find array start in index-data.js');
  ...
  return (
    content.slice(0, arrayStart + 1) + '\n' + templateJson + ',' + content.slice(arrayStart + 1)
  );
}
...
function createResultData(templateData: any): string {
  ...
  const report = {
    ...
    upstreamTemplate:
      typeof templateData.originUpstream === 'string' && templateData.originUpstream.includes('/')
...
function createDashboardHtml(templateData: any, dataFileName: string): string {
  ...
  const gaugeClass = percent >= 80 ? 'high' : percent >= 50 ? 'medium' : 'low';
...
function createLatestJson(templateData: any, dataFileName: string, dashboardFileName: string) {
  return {
    repoUrl: templateData.repoUrl,
    ruleSet: templateData.ruleSet || 'default',
    timestamp: templateData.timestamp,
...
function createHistoryEntry(templateData: any, dataFileName: string, dashboardFileName: string) {
  return {
    timestamp: templateData.timestamp,
    ruleSet: templateData.ruleSet || 'default',
    percentage: templateData.compliance.percentage,
...
function buildPrBody(t: any, folderPath: string, dashboardFile: string, dataFile: string): string {
