...
/**
 * POST /api/v4/submit-analysis-dispatch
 * Dispatches a repository_dispatch event to trigger analysis submission workflow
 *
 * Body:
 * - event_type: Event type for repository_dispatch (required)
 * - client_payload: Payload data for the workflow (required)
 *   - targetRepo or repoSlug: Target repository (optional, falls back to env)
 *   - Additional payload data
 *
 * Returns:
 * - 204 No Content on success
 * - Header: x-template-doctor-repo-slug with target repository
 */
router.post(
        ...
        try {
            ...
            if (!event_type || !client_payload) {
                return res.status(400).json({
                    error: "Missing event_type or client_payload",
                    requestId,
                });
            }

            // Repo slug resolution precedence:
            ...
            console.log("[submit-analysis-dispatch] dispatching", {
                requestId,
                event_type,
                repoSlug,
            });
            ...
            if (!response.ok) {
                ...
                console.error(
                    "[submit-analysis-dispatch] GitHub dispatch failed",
                    {
                        requestId,
            ...
            // Return 204 with custom header (preserves legacy behavior)
            res.setHeader("x-template-doctor-repo-slug", repoSlug);
            res.status(204).send();
        } catch (error: any) {
            console.error("[submit-analysis-dispatch] exception", {
                requestId,
                error: error?.message,
            });
            next(error);
