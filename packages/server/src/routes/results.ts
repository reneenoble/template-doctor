 ...
 */
router.get('/results/latest', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    console.error('[Results API] Latest results failed:', error?.message);
    next(error);
  }
});
 ...
 */
router.get('/results/leaderboard', async (req: Request, res: Response, next: NextFunction) => {
  try {
 ...
 */
router.get('/results/repo/:owner/:repo', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.json({
      ...
      analyses: analyses.map(a => {
        ...
        const categories = a.categories || buildCategoriesFromIssues(
 ...
 */
router.get('/results/search', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (minCompliance) {
      filter['compliance.percentage'] = { $gte: parseFloat(minCompliance as string) };
    }
