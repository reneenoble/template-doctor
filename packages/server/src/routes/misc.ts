...
const batches: Record<string, { items: BatchScanItem[]; created: string; mode?: string }> = {};
  ...
  try {
    const { repos, mode } = req.body || {};
      ...
      if (sanitized.length === 0) {
        return res.status(400).json({ error: 'No valid repo slugs provided' });
      }
...
async function processAsync(batchId: string, req: Request) {
  ...
  for (const item of batch.items) {
    ...
    try {
      ...
      const analysisResult = await runSingleAnalysis(req, item.repo, batch.mode || 'azd');
...
router.get('/batch-scan-status', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/repo-fork', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    try {
      ...
      console.log('Failed to identify token user:', err instanceof Error ? err.message : err);
    }

    const forkOwner =
      targetOwner || authedUser || process.env.FORK_FALLBACK_OWNER || 'fallback-user';
    ...
    try {
      ...
      const isSamlError =
        err.status === 403 && (err.message?.includes('SAML') || err.message?.includes('SSO'));
      ...
      if (isSamlError) {
        ...
        return res.status(403).json({
          ...
          samlRequired: true,
          documentationUrl: err.response?.headers?.['x-github-sso'] || undefined,
          requestId,
        });
      }
...
const AZURE_OPENAI_DEPLOYMENT =
  process.env.AZURE_OPENAI_DEPLOYMENT || process.env.AZURE_OPENAI_MODEL || DEFAULT_MODEL;
...
const CACHE_TTL_MS = parseInt(process.env.ISSUE_AI_CACHE_TTL_MS || '300000', 10);
const MAX_CACHE_ENTRIES = parseInt(process.env.ISSUE_AI_CACHE_MAX || '500', 10);
const aiCache = new Map<string, { body: string; title: string; expires: number }>();

function cacheSet(key: string, value: { body: string; title: string; expires: number }) {
...
// Rate limiting
const RATE_LIMIT_WINDOW_MS = parseInt(process.env.ISSUE_AI_RATE_LIMIT_WINDOW_MS || '60000', 10);
const RATE_LIMIT_MAX = parseInt(process.env.ISSUE_AI_RATE_LIMIT_MAX || '20', 10);
...
router.post('/issue-ai-proxy', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (PROVIDER === 'azure') {
      if (!AZURE_OPENAI_ENDPOINT)
        return res.status(400).json({ error: 'AZURE_OPENAI_ENDPOINT not set' });
      if (!AZURE_OPENAI_DEPLOYMENT)
        return res.status(400).json({ error: 'AZURE_OPENAI_DEPLOYMENT not set' });
      if (!AZURE_OPENAI_API_KEY)
        return res.status(400).json({ error: 'AZURE_OPENAI_API_KEY not set' });
    } else {
    ...
    if (rl.limited) {
      return res.status(429).json({
        ...
        retryInSeconds: rl.retryIn ? Math.ceil(rl.retryIn / 1000) : undefined,
      });
    }
