...
configRouter.get('/client-settings', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const requireAuthForResults = getMergedValue(
      'REQUIRE_AUTH_FOR_RESULTS',
      process.env.REQUIRE_AUTH_FOR_RESULTS || process.env.TD_REQUIRE_AUTH_FOR_RESULTS || 'false',
    );
    ...
    const archiveCollection = getMergedValue(
      'ARCHIVE_COLLECTION',
      process.env.TD_ARCHIVE_COLLECTION || process.env.ARCHIVE_COLLECTION || 'gallery',
    );

    const dispatchTargetRepo = getMergedValue(
      'DISPATCH_TARGET_REPO',
      process.env.DISPATCH_TARGET_REPO || process.env.TD_DISPATCH_TARGET_REPO || '',
    );
