...
class AnalysisStorageService {
    ...
    async saveAnalysis(
        ...
        try {
            ...
            const issuesArray = Array.isArray(data.compliance.issues)
                ? data.compliance.issues
                : [];
            ...
            console.log(
                `[AnalysisStorage] Saved analysis for ${data.repoUrl} with ID ${analysisId}`,
            );
            ...
            console.log(
                `[AnalysisStorage] Updated repos collection for ${data.repoUrl}`,
            );
    ...
    async getLatestAnalyses(limit: number = 50): Promise<Repo[]> {
        try {
            ...
            console.error(
                "[AnalysisStorage] Get latest failed:",
                error?.message,
            );
            throw error;
        }
    ...
    async getAnalysesByRepo(repoUrl: string): Promise<Analysis[]> {
        try {
            ...
            console.error(
                "[AnalysisStorage] Get by repo failed:",
                error?.message,
            );
            throw error;
        }
    ...
    async getAnalysisById(id: string): Promise<Analysis | null> {
        try {
            ...
            console.error(
                "[AnalysisStorage] Get by ID failed:",
                error?.message,
            );
            return null;
        }
    ...
    > {
        try {
            ...
            const leaderboard = await Promise.all(
                results.map(async (repo) => {
                    ...
                    return {
                        ...
                        repo: repo.repo,
                        compliance:
                            repo.latestAnalysis?.compliancePercentage || 0,
            ...
            console.error(
                "[AnalysisStorage] Leaderboard failed:",
                error?.message,
            );
            throw error;
        }
