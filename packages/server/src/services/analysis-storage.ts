  ...
  async saveAnalysis(data: AnalysisData): Promise<{ id: string; success: boolean }> {
    try {
      ...
      const issuesCount = Array.isArray(data.compliance.issues) 
        ...
        : (typeof data.compliance.issues === 'number' ? data.compliance.issues : 0);
      ...
      const count = await database.analysis.countDocuments({ repoUrl: data.repoUrl });
      if (count > 10) {
        ...
        if (toDelete.length > 0) {
          ...
          console.log(`[AnalysisStorage] Pruned ${toDelete.length} old analyses for ${data.repoUrl}`);
        }
      }
    ...
    try {
      ...
      const leaderboard = await Promise.all(
        results.map(async (repo) => {
          const scanCount = await database.analysis.countDocuments({ repoUrl: repo.repoUrl });
          return {
