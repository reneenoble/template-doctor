    ...
    async connect() {
        ...
        if (mongoUri) {
            ...
            try {
                ...
                console.log(
                    `[Database] Connected to local MongoDB database: ${databaseName}`,
                );
                ...
                console.error(
                    "[Database] Local MongoDB connection failed:",
                    error?.message,
                );
                throw error;
            }
        ...
        if (cosmosEndpoint) {
            console.log(
                `[Database] Connecting to Cosmos DB: ${cosmosEndpoint}`,
            );
            try {
                ...
                this.isConnected = true;
                console.log(
                    `[Database] Connected to Cosmos DB database: ${databaseName}`,
                );
                // Create indexes (async, non-blocking)
                this.createIndexes().catch((err) =>
                    console.error("[Database] Index creation failed:", err),
                ...
                this.isConnected = false;
                console.error(
                    "[Database] Cosmos DB connection failed:",
                    error?.message,
                );
                throw error;
            }
