...
export async function downloadValidationArtifact(
  ...
  try {
    ...
    if (!artifactsResponse.ok) {
      console.error('[azd-validation] fetch artifacts failed', { status: artifactsResponse.status, runId });
      return null;
    }
    const artifactsData = await artifactsResponse.json();
    const validationArtifact = artifactsData.artifacts?.find((a: any) => a.name.endsWith('-validation-result'));
    ...
    if (!downloadResponse.ok) {
      console.error('[azd-validation] artifact download failed', { status: downloadResponse.status, artifactId: validationArtifact.id });
      return null;
    }
    ...
    let resultEntry = entries.find(e => !e.isDirectory && e.entryName.endsWith('.md'))
      || entries.find(e => !e.isDirectory && e.entryName.endsWith('.log'))
      || entries.find(e => !e.isDirectory);

    if (!resultEntry) {
      console.error('[azd-validation] no result file in artifact ZIP', { artifactId: validationArtifact.id, entries: entries.map(e => e.entryName) });
      return null;
    }

    return resultEntry.getData().toString('utf8');
  } catch (error) {
    console.error('[azd-validation] artifact processing error', { error, runId });
    return null;
  }
}
 ...
 */
export function parseAzdValidationResult(markdown: string): AzdValidationResult {
  const azdUpSuccess = /- \[x\].*azd up/i.test(markdown) || /:white_check_mark:.*azd up/i.test(markdown);
  const azdDownSuccess = /- \[x\].*azd down/i.test(markdown) || /:white_check_mark:.*azd down/i.test(markdown);
