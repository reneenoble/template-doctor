...
class DatabaseService {
  ...
  async connect(): Promise<void> {
    ...
    const databaseName = process.env.COSMOS_DATABASE_NAME || process.env.MONGODB_DATABASE || 'template_doctor';
    ...
    throw new Error('No database configuration found. Set MONGODB_URI for local MongoDB or COSMOS_ENDPOINT for Cosmos DB');
  }
  ...
  private async createIndexes(): Promise<void> {
    ...
    try {
      ...
      await this.repos.createIndex({ 'latestAnalysis.compliancePercentage': -1 }, { background: true });
   ...
   */
  async healthCheck(): Promise<{ connected: boolean; type?: string; database?: string; latency?: number; error?: string }> {
    if (!this.db) {
