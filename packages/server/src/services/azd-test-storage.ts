...
class AzdTestStorage {
    ...
    async saveAzdTest(params: SaveAzdTestParams): Promise<string> {
        ...
        const insertResult = await database.azdTests.insertOne(
            azdTest as AzdTest,
        );
            ...
            { repoUrl: params.repoUrl },
            {
                $set: {
                    latestAzdTest: {
                        testId: insertedId,
                        timestamp:
                            params.completedAt || params.startedAt || now,
                        status: params.status,
                        duration: params.duration,
                        result: params.result,
                    },
        ...
        console.log(
            `[AzdTestStorage] Saved test ${insertedId} for ${params.repoUrl}, status: ${params.status}`,
        );
        return insertedId;
    }
    ...
    async updateAzdTest(
        ...
        if (updates.status || updates.result) {
            const test = await database.azdTests.findOne({
                _id: testId as any,
            });
            if (test) {
                await database.repos.updateOne(
                    ...
                    {
                        $set: {
                            "latestAzdTest.status":
                                updates.status || test.status,
                            "latestAzdTest.duration":
                                updates.duration || test.duration,
                            "latestAzdTest.result":
                                updates.result || test.result,
                            "latestAzdTest.timestamp":
                                updates.completedAt ||
                                test.completedAt ||
                                test.startedAt,
                            updatedAt: now,
        ...
        console.log(
            `[AzdTestStorage] Updated test ${testId}, status: ${updates.status || "unchanged"}`,
        );
    }
