...
function log(msg) {
    process.stdout.write(`[playwright-verify] ${msg}\n`);
}
...
const allowMissing = /^(1|true|yes)$/i.test(
    process.env.PLAYWRIGHT_ALLOW_MISSING || "",
);
const skipDownload = /^(1|true|yes)$/i.test(
...
if (allowMissing) {
    log(
        "Chromium missing but PLAYWRIGHT_ALLOW_MISSING is set; proceeding (tests may skip e2e).",
    );
    process.exit(0);
}
