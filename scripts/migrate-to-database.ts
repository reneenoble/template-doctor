...
function convertToAnalysisDocument(data: WindowReportData, filePath?: string): AnalysisDocument {
  ...
  return {
    ...
    compliance: {
      ...
      passed: Array.isArray(data.compliance.compliant) ? data.compliance.compliant.length : 0,
      issues: Array.isArray(data.compliance.issues) ? data.compliance.issues.length : 0,
    },
