...
async function migrateV1ToV2() {
    ...
    try {
        ...
        const repoDocs = repoGroups.map((group) => {
        ...
        for (const repoDoc of repoDocs) {
            const result = await database.repos.updateOne({ repoUrl: repoDoc.repoUrl }, { $set: repoDoc }, { upsert: true });
