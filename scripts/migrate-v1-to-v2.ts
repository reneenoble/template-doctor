...
async function migrateV1ToV2() {
    ...
    try {
        ...
        const repoGroups = await database.analysis
            .aggregate([
                {
                    $sort: { scanDate: -1 },
                },
        ...
        console.log(
            "🧹 Step 3: Pruning old analyses (keeping last 10 per repo)...",
        );
        let totalDeleted = 0;
        ...
        for (const group of repoGroups) {
            ...
            if (count > 10) {
                ...
                console.log(
                    `   Pruned ${deleteResult.deletedCount} old analyses from ${repoUrl}`,
                );
            }
        }

        console.log(
            `   ✅ Deleted ${totalDeleted} old analyses (kept last 10 per repo)\n`,
        );
        ...
        console.log(
            `   Analysis collection: ${analysisCount} documents (max 10 per repo)\n`,
        );
        ...
        const retentionStats = await database.analysis
            .aggregate([
                {
                    $group: {
                        _id: "$repoUrl",
        ...
        console.log(
            `   - Retained ${analysisCount} analysis documents (max 10 per repo)`,
        );
