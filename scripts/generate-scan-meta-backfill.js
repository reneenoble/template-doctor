...
function findRepoDirs(base) {
    return (
        fs
            ...
            .filter(
                (name) => !["assets"].includes(name) && !name.startsWith("."),
            )
    );
}
...
function buildMetaForRepo(slug) {
    ...
    if (!fs.existsSync(latestPath)) {
        ...
        const percentageMatch =
            raw.match(/"percentage"\s*:\s*(\d+)/) ||
            raw.match(/"percentageCompliant"\s*:\s*(\d+)/);
    ...
    try {
        ...
        if (
            ...
            return null; // ruleSet can be synthesized
        ...
        if (!ruleSet) {
            console.warn(
                "[backfill] ruleSet missing in latest.json for",
                slug,
                "defaulted to dod",
            );
        }
