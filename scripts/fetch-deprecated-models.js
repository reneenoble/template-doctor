        ...
        const request = https
            .get(url, (res) => {
                ...
                if (
                    ...
                    fetchHtml(fullRedirectUrl).then(resolve).catch(reject);
                    return;
                }
 ...
 */
function extractDeprecatedModels(
    html,
    includeFuture = false,
    ...
    tables.forEach((table, tableIndex) => {
        ...
        if (headerRow.includes("<th") || headerRow.includes("</th>")) {
            ...
            console.log(
                `Identified column indices - Model: ${modelColumnIndex}, Legacy: ${legacyDateColumnIndex}, Deprecation: ${deprecationDateColumnIndex}, Retirement: ${retirementDateColumnIndex}`,
            );
        }
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            if (cells.length > modelColumnIndex) {
                ...
                if (modelNameHtml.includes("<a")) {
                    const linkMatch = modelNameHtml.match(
                        /<a[^>]*>([\s\S]*?)<\/a>/,
                    );
                ...
                const processDateCell = (columnIndex, dateName) => {
                    if (columnIndex >= 0 && cells.length > columnIndex) {
                        ...
                        if (
                            dateText.toLowerCase().includes("deprecated") ||
                            ...
                            console.log(
                                `  Found text indicating model is already deprecated in ${dateName} column`,
                            );
                        ...
                        try {
                            ...
                            if (!isNaN(potentialDate.getTime())) {
                                console.log(
                                    `  Parsed ${dateName} date: ${potentialDate.toISOString().split("T")[0]}`,
                                );
                                return { date: potentialDate };
                            }
                        } catch (e) {
                            console.log(
                                `  Could not parse ${dateName} date: "${dateText}"`,
                            );
                        }
                    }
                    return null;
                ...
                const retirementResult = processDateCell(
                    retirementDateColumnIndex,
                    "Retirement",
                );
                const deprecationResult = processDateCell(
                    deprecationDateColumnIndex,
                    "Deprecation",
                );
                const legacyResult = processDateCell(
                    legacyDateColumnIndex,
                    "Legacy",
                );

                // Helper function to determine deprecation status from multiple date results
                const determineDeprecationStatus = (
                    dateResults,
                    today,
                    includeFuture,
                ) => {
                    ...
                    for (const [type, result] of Object.entries(dateResults)) {
                        if (result?.isDeprecatedByText) {
                            console.log(
                                `  Model is explicitly marked as deprecated in ${type} column text`,
                            );
                            return {
                                isDeprecated: true,
                                relevantDate: null,
                                reason: `explicit ${type} text`,
                            };
                        }
                    }
                    ...
                    const datesByPrecedence = [
                        {
                            type: "Retirement",
                            date: dateResults.retirement?.date,
                        },
                        {
                            type: "Deprecation",
                            date: dateResults.deprecation?.date,
                        },
                    ...
                    for (const item of datesByPrecedence) {
                        const isDateDeprecated =
                            item.date <= today || includeFuture;
                        if (isDateDeprecated) {
                            console.log(
                                `  Model is deprecated based on ${item.type} date: ${item.date.toISOString().split("T")[0]}`,
                            );
                            return {
                                isDeprecated: true,
                    ...
                    if (datesByPrecedence.length > 0) {
                        ...
                        console.log(
                            `  Model will be deprecated on ${earliestDate.type} date: ${earliestDate.date.toISOString().split("T")[0]}`,
                        );
                        return {
                            isDeprecated: includeFuture, // Only mark as deprecated if includeFuture is true
                            relevantDate: earliestDate.date,
                            reason: includeFuture
                                ? `future ${earliestDate.type} date ${earliestDate.date.toISOString().split("T")[0]}`
                                : "not yet deprecated",
                        };
                    }
                ...
                console.log(
                    `  Is deprecated: ${isDeprecated}, Reason: ${status.reason}, Include future: ${includeFuture}`,
                );

                console.log(
                    `  Is deprecated: ${isDeprecated}, Include future: ${includeFuture}`,
                );
                ...
                if (
                    isDeprecated &&
                    modelName &&
                    (includeAllModels || matchesPattern)
                ) {
                    console.log(
                        `  Adding model to deprecated list: ${modelName}`,
                    );
                    ...
                    console.log(
                        `  ⚠️ Model "${modelName}" is deprecated but doesn't match our naming patterns - SKIPPING`,
                    );
                    if (includeAllModels) {
                        console.log(
                            `  Including model anyway due to --include-all-models flag`,
                        );
                        tableModels.push(modelName);
                        deprecatedModels.push(modelName);
                    }
        ...
        console.log(
            `Table ${tableIndex + 1}: Found ${tableModels.length} deprecated models`,
        );
    });
    ...
    modelsPerTable.forEach((tableData) => {
        console.log(
            `Table ${tableData.tableIndex + 1}: ${tableData.count} models found`,
        );
    ...
    if (deprecatedModels.length !== uniqueModels.length) {
        console.log(
            `Removed ${deprecatedModels.length - uniqueModels.length} duplicate models`,
        );
    }
...
function updateConfig(models, dryRun = false) {
    ...
    try {
        ...
        if (!fs.existsSync(configPath)) {
            ...
            console.log(
                `"deprecatedModels": ${JSON.stringify(models, null, 2)}`,
            );
            return;
        }
        ...
        console.log(
            `Updated config.json with ${models.length} deprecated models.`,
        );
    } catch (error) {
...
async function main() {
    try {
        ...
        const deprecatedModels = extractDeprecatedModels(
            html,
            includeFuture,
            includeAllModels,
        );
