...
try {
    ...
    console.log(`⚠️  No .env.local found at ${serverEnvPath}, trying root .env...`);
 ...
 */
async function findResultFiles(resultsDir) {
    const files = [];
 ...
 */
function convertToAnalysisDocument(data) {
    ...
    return {
        ...
        issues: Array.isArray(data.compliance.issues) ? data.compliance.issues : [],
        compliant: Array.isArray(data.compliance.compliant) ? data.compliance.compliant : [],
        analysisResult: {
 ...
 */
async function exportToBSON(documents, outputDir) {
...
`;
    ...
    // Generate statistics
    const timestamps = documents.map(d => new Date(d.timestamp).getTime()).filter(t => !isNaN(t));
    const stats = {
        ...
        complianceStats: {
            average: documents.reduce((sum, d) => sum + d.compliance.percentage, 0) / documents.length,
    ...
    if (stats.dateRange) {
        console.log(`   Date range: ${stats.dateRange.earliest.toISOString()} to ${stats.dateRange.latest.toISOString()}`);
    }
    console.log(`   Average compliance: ${stats.complianceStats.average.toFixed(2)}%`);
}
/**
            ...
            try {
                ...
                for (const doc of batch) {
                    try {
                        ...
                        console.error(`   Failed to insert ${doc.repoUrl}: ${docError?.message}`);
                    }
                }
            }
        }
        console.log(`✅ Successfully inserted ${inserted}/${documents.length} documents`);
        // Verify insertion
...
async function migrate() {
    ...
    for (const file of files) {
        ...
        if (data) {
            const doc = convertToAnalysisDocument(data);
            documents.push(doc);
            console.log(`   ✓ ${data.repoUrl} (${new Date(data.timestamp).toISOString()})`);
        }
    }
    ...
    if (mode === 'export') {
        ...
        console.log(`   mongoimport --uri '<connection-string>' --db template-doctor --collection analysis --file migration-output/analysis-export.json`);
        ...
        console.log('Usage: node scripts/migrate-to-database.js [export|import]');
        process.exit(1);
    }
}
// Run migration
