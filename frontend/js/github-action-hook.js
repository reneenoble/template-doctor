...
async function submitAnalysisToGitHub(result, username) {
    ...
    try {
        ...
        console.log('Analysis data being submitted:', {
            ...
            complianceStats: {
                percentage: result.compliance.compliant.find(c => c.id === 'compliance-summary')?.details?.percentageCompliant || 0,
        ...
        const apiUrl = window.config?.githubActionWebhookUrl || `https://api.github.com/repos/${targetRepo}/dispatches`;
        ...
        if (!token) {
            ...
            return { 
                success: false, 
                error: 'No GitHub token available. Please ensure you are logged in with a GitHub account that has write access to the repository.',
                details: 'The GitHub token is required to create a pull request with your analysis results.'
            };
        }
        ...
        if (!response.ok) {
            ...
            if (response.status === 404) {
                throw new Error(`Repository not found (404): The repository '${targetRepo}' either doesn't exist or you don't have access to it.`);
            } else if (response.status === 403) {
                throw new Error(`Permission denied (403): Your GitHub token doesn't have permission to trigger workflows. Please check that your token has the 'repo' scope.`);
            } else if (response.status === 401) {
                throw new Error(`Unauthorized (401): Your GitHub authentication token is invalid or expired. Please log out and log in again.`);
            } else {
