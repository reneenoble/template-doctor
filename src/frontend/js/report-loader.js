...
(function (window) {
  ...
  const RESULTS_DIR =
    typeof window.RESULTS_DIR === 'string' && window.RESULTS_DIR ? window.RESULTS_DIR : 'results';
  ...
  const ReportLoader = {
      ...
      if (dataJsPath) {
        ...
        this._loadDataJsFile(dataJsPath)
          ...
          .catch((error) => {
            debug(
              'report-loader',
              `Failed to load data.js, falling back to other strategies: ${error.message}`,
            );
      ...
      return new Promise((resolve, reject) => {
        ...
        script.onload = function () {
          ...
          if (window.reportData) {
            ...
            console.log(
              `[ReportLoader] Found reportData with keys:`,
              Object.keys(window.reportData),
            );
        ...
        script.onerror = function (error) {
          console.error(
            `[ReportLoader] Failed to load script: /${RESULTS_DIR}/${dataJsPath}`,
            error,
          );
        ...
        setTimeout(() => {
          if (window.reportData) {
            ...
            console.log(
              `[ReportLoader] Found reportData after timeout with keys:`,
              Object.keys(window.reportData),
            );
            const data = window.reportData;
            // Store the current data
            ...
            console.error(
              `[ReportLoader] Timed out waiting for reportData. Script may have failed to execute properly.`,
            );
            ...
            fetch(`/${RESULTS_DIR}/${dataJsPath}`)
              ...
              .then((text) => {
                console.log(
                  `[ReportLoader] File content fetched (first 100 chars): ${text.substring(0, 100)}...`,
                );
              ...
              .catch((error) => {
                ...
                reject(
                  new Error(
                    `Timed out waiting for reportData and couldn't fetch file: ${error.message}`,
                  ),
                );
              });
          }
        ...
        if (templatePath) {
          ...
          this._fetchReportFile(`/${RESULTS_DIR}/${templatePath}`)
            ...
            .catch((error) => {
              debug(
                'report-loader',
                `Failed loading from specific path, continuing with other strategies`,
                error,
              );
          ...
          this._fetchReportFile(`/${RESULTS_DIR}/${template}/latest.json`)
            .then((data) => {
              if (isValidObject(data)) {
                ...
                if (data.dataPath) {
                  debug(
                    'report-loader',
                    `Found dataPath in latest.json: ${data.dataPath}, loading that file`,
                  );
                  return this._loadDataJsFile(`${template}/${data.dataPath}`).catch((err) => {
                    debug(
                      'report-loader',
                      `Failed to load data.js file: ${err.message}, using latest.json metadata`,
                    );
                    return data;
                  });
                }
            ...
            .then((data) => {
              if (isValidObject(data)) {
                ...
                throw new Error(
                  'Invalid data after loading from latest.json or its referenced data.js',
                );
              }
            })
            .catch((error) => {
      ...
      return this._fetchReportFile(`/${RESULTS_DIR}/${folderPath}/index.json`)
        .then((indexData) => {
          ...
          if (indexData && Array.isArray(indexData.timestamps) && indexData.timestamps.length > 0) {
            debug(
              'report-loader',
              `Found index.json with ${indexData.timestamps.length} timestamps`,
            );
    ...
    _tryTimestamps: function (template, timestamps) {
      ...
      return this._fetchReportFile(path).catch((error) => {
