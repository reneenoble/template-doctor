      ...
      window.GitHubClient = {
        ...
        createIssueWithoutCopilot: async (_o, _r, title, body, labels) => {
          const child = {
            ...
            body,
            labels,
            number: 200 + window.__createdChildren.length,
            id: 'CHILD_NODE',
            url: 'http://y',
          };
...
test.describe('GitHub issue creation labels and body metadata', () => {
  test('main issue includes ruleset label and body Configuration/Severity Breakdown', async ({
    page,
  }) => {
    ...
    expect(ensuredFlat).toEqual(
      expect.arrayContaining(['severity:high', 'severity:medium', 'severity:low']),
    );
  ...
  test('child issues include severity and ruleset labels plus Context section', async ({
    page,
  }) => {
    await setupPageWithReport(page, {
      ruleSet: 'custom',
    ...
    await page.waitForFunction(
      () => Array.isArray(window.__createdChildren) && window.__createdChildren.length === 3,
    );
    ...
    expect(children[0].body).toMatch(
      /Rule Set: Custom \(custom from https:\/\/gist\.github\.com\/user\/abcd1234\)/,
    );
  });
});
